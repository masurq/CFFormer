...
class LightHamHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        inputs = [F.interpolate(level, size=inputs[0].shape[2:], mode='bilinear', align_corners=False) for level in
                  inputs]
...
if __name__ == '__main__':
    ...
    model = model.cuda()
    x = [torch.zeros(1, 64, 256, 256), torch.ones(1, 128, 128, 128), torch.ones(1, 320, 64, 64) * 2,
         torch.ones(1, 512, 32, 32) * 3]
