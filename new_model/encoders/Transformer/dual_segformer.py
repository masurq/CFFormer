...
from models.new_model.net_utils import FeatureFusionModule as FFM
from models.new_model.net_utils import FeatureRectifyModule as FCM
...
class DualSegFormer(nn.Module):
    ...
    def forward_features(self, x1, x2):
        ...
        # B H*W/16 C
        x2, _, _ = self.extra_patch_embed1(x2)
        ...
        x_fused = self.FFMs[0](x1, x2)
        outs.append(x_fused)

        # stage 2
        x1, H, W = self.patch_embed2(x1)
        x2, _, _ = self.extra_patch_embed2(x2)
        for i, blk in enumerate(self.block2):
            x1 = blk(x1, H, W)
        for i, blk in enumerate(self.extra_block2):
            x2 = blk(x2, H, W)
        x1 = self.norm2(x1)
        x2 = self.extra_norm2(x2)

        x1 = x1.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x2 = x2.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x1, x2 = self.FCMs[1](x1, x2)
        x_fused = self.FFMs[1](x1, x2)
        outs.append(x_fused)

        # stage 3
        x1, H, W = self.patch_embed3(x1)
        x2, _, _ = self.extra_patch_embed3(x2)
        for i, blk in enumerate(self.block3):
            x1 = blk(x1, H, W)
        for i, blk in enumerate(self.extra_block3):
            x2 = blk(x2, H, W)
        x1 = self.norm3(x1)
        x2 = self.extra_norm3(x2)

        x1 = x1.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x2 = x2.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x1, x2 = self.FCMs[2](x1, x2)
        x_fused = self.FFMs[2](x1, x2)
        outs.append(x_fused)

        # stage 4
        x1, H, W = self.patch_embed4(x1)
        x2, _, _ = self.extra_patch_embed4(x2)
        for i, blk in enumerate(self.block4):
            x1 = blk(x1, H, W)
        for i, blk in enumerate(self.extra_block4):
            x2 = blk(x2, H, W)
        x1 = self.norm4(x1)
        x2 = self.extra_norm4(x2)

        x1 = x1.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x2 = x2.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x1, x2 = self.FCMs[3](x1, x2)
        x_fused = self.FFMs[3](x1, x2)
        outs.append(x_fused)
