...
import numpy as np
from models.new_model.encoders.Transformer.ViTAE.token_transformer import Token_transformer
from models.new_model.encoders.Transformer.ViTAE.token_performer import Token_performer
from models.new_model.encoders.Transformer.ViTAE.window import WindowTransformerBlock, window_partition, window_reverse
...
class ReductionCell(nn.Module):
    def __init__(self, img_size=224, in_chans=3, embed_dims=64, token_dims=64, downsample_ratios=4, kernel_size=7,
                 ...
                 drop=0., attn_drop=0., drop_path=0., mlp_ratio=1.0, gamma=False, init_values=1e-4, SE=False,
                 window_size=7):
        super().__init__()
        ...
        if tokens_type == 'performer':
            ...
            self.attn = WindowTransformerBlock(in_dim=in_chans, out_dim=token_dims, input_resolution=(
            self.img_size // self.downsample_ratios, self.img_size // self.downsample_ratios),
                                               num_heads=num_heads, mlp_ratio=mlp_ratio, drop=drop,
                                               attn_drop=attn_drop, drop_path=drop_path, window_size=window_size,
                                               shift_size=0)
